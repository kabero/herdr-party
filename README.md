...
- The number of instructions you have given the session appears next to the name, like `Sota [19]`. It is counted from the Claude Code conversation log (`~/.claude/projects/*/<session-id>.jsonl`, user messages excluding tool results), read incrementally, so it is exact even across viewer restarts. If Herdr has no session ID for a guest (for example when the Claude integration hook is outdated; check `herdr integration status`), the log is picked from the directory for that guest's cwd (`~/.claude/projects/<cwd with non-alphanumerics replaced by ->`), taking the most recently modified log not used by another guest. Sessions without a log show no number.
...
- A stage spans the venue width, and performers stand centered on it with equal spacing. It is one row of platform with the name embedded in it, and captions under that.
