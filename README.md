...
- ステージの名前の横に、出演者の cwd から取った git の状況を `master ✱3 ↑1` の形で出します（ブランチ、コミットしていない変更の数、リモートとの差）。Herdr の worktree は space ごとに分かれるので、worktree のステージにはそのブランチが出ます。同じ space の中で別のリポジトリやブランチにまたがっているときは、相異なるものを並べます。
