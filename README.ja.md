...
┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈
   ...
   ▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄
                   master ✱3                     feat/x ✱1
    > ステージを縦並び    > README を英語版  > 落ちているテス
      にして              にして             トを直して
                          < 英語版を書いて相  ? Bash command: …
```
...
- 名前の横に、そのセッションに出した指示の回数が `Sota [19]` のように出ます。Claude Code の会話ログ（`~/.claude/projects/*/<セッション ID>.jsonl`）のユーザー発言（ツール結果を除く）を差分で数えるので、ビューアを開き直しても正確です。Herdr がセッション ID を持っていない guest（Claude の連携フックが古いときなど。`herdr integration status` で確認できます）は、その cwd のログ置き場（`~/.claude/projects/<cwd の英数字以外を - にした名前>`）から、そのペインの claude プロセスの起動より後に始まり、かつ最後のユーザー発言がペインの画面に出ている最後のプロンプトと一致するログを割り当てます。一致するものがなければ、推測で出さずに空にします。ログが見つからないセッションには出しません。
...
- ステージは会場の幅いっぱいで、出演者は等間隔で中央に立ちます。1 行の台で、上に帯、下にキャプションが付きます。
- 1 列に収まらないときはステージを会場いっぱいまで広げ、それでも余る人は後ろの列（上側）に並びます。
- ステージは git のリポジトリ単位です。同じリポジトリの worktree（Herdr では別の space）は 1 つのステージに立ち、worktree ごとの区画を仕切り `┆` で分けます。各区画の下にそのブランチと変更数（`master ✱3` の形。ブランチとコミットしていない変更の数）が出て、ステージ名はリポジトリ名です。区画をクリックするとその worktree の space に移ります。
- 区画が横に収まらないときは、区画ごとに別のステージ（名前は同じリポジトリ名）に分けます。git 管理外の space は、space のラベルがそのままステージ名になります。
- ステージの色は space ごとに変わります。

### 足元のキャプション

ステージ上の各人の真下、その人の列に、キャプションが付きます。`> …` はそのセッションに最後に投げたプロンプト（3 行まで）で、会話ログから差分で拾うので、新しい指示を送った瞬間に変わります。完了した人には `< …` で終わった返答の冒頭、承認待ちの人には `? …` で承認ダイアログや質問の内容も出ます。人数が少ないときは席を広げる（最大 26 桁）ので、キャプションの幅も広がります。リポジトリ名と git の状況は棒人間のすぐ上の帯に出ます（ステージと同じ色の背景に黒文字）。worktree の区画のブランチは台の下の行に出ます。
...
| `Enter` / `o` | 選んだ guest のセッションを開く。何も選んでいなければ、いま最も対応が必要な guest（承認待ち → 完了、待ちが長い順の先頭）へ飛ぶ。そういう guest もいなければ、一連のジャンプを始めた場所（最初に飛ぶ前にいたペイン）へ戻り、その起点を忘れる。その guest は頭上に上下する `▼` が出て名前が状態色の太字になり、上部の一覧では `⏎` が付く |
| `delete`（Mac の刻印。Backspace のこと。`-` `b` も可） | 直前にいた場所へ戻る。直前にフォーカスされていたペイン（agent がいればその agent、いなければそのタブ）へ移る。履歴は `pane.focused` イベントから取るので、会場以外での移動も含む |
| `Esc` | 選択を解除 |
| `?` | 操作の説明を出す / 消す |
| `q` | 終了（ペインも閉じる） |
