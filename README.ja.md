...
ステージ名の下に、働いている人ごとに `Sota ▸ Relaunch the pane and commit` のような 1 行が出ます。Claude Code がツール実行のたびに書き残す説明文（Bash の説明、編集や検索ならツール名とファイル名）を会話ログから差分で拾うので、作業が進むにつれてリアルタイムに変わり、新しい指示を出すと消えます。
