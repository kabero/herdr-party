...
ステージ上の各人の真下、その人の列に、キャプションが付きます。`> …` はそのセッションに最後に投げたプロンプト（2 行）で、会話ログから差分で拾うので、新しい指示を送った瞬間に変わります。完了した人には `< …` で終わった返答の冒頭、承認待ちの人には `? …` で承認ダイアログや質問の内容も出ます。人数が少ないときは席を広げる（最大 26 桁）ので、キャプションの幅も広がります。リポジトリ名と git の状況はステージの上の見出しに、worktree の区画のブランチはステージの下に出ます。
